    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < viewSize; i++) {
            for (int j = 0; j < viewSize; j++) {
                g2D.drawImage(GameManager.getWalkingManager().getWalking().getMap().getPlace(j+ GameManager.getWalkingManager().getWalking().fogOfWar.getMinX(), i+ GameManager.getWalkingManager().getWalking().fogOfWar.getMinY()).getImage(), j * scale, i * scale, scale, scale, null);
            }
        }
    }
}
